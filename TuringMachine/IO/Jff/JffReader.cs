...
namespace FEI.TuringMachineSimulator.IO.Jff {
	public class JffReader
    {
        ...
        public VirtualTuringMachine Read()
        {
            try
            {
                using (XmlTextReader xr = new XmlTextReader(FileName))
                {
                    VirtualTuringMachine result = new VirtualTuringMachine();
                        ...
                        switch (xr.NodeType)
                        {
                            ...
                            case XmlNodeType.Text:
                                if (curElement.StartsWith("."))
                                {
                                    ...
                                    {
                                        case "type":
                                            if (xr.Value.Trim() != "turing") return null;
                                            break;
                                        case "state.label":
                                        ...
                                        case "automaton.transition.write":
                                            if (xr.Value.Trim() == "")
                                                curTransition.WriteSymbol = InfiniteTape.Blank;
                                            else
                                                curTransition.WriteSymbol = xr.Value;                                             
                                            break;
                                        case "transition.move":
                                        case "automaton.transition.move":
