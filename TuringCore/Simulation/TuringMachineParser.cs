...
namespace FEI.TuringCore.Simulation {
	public class TuringMachineParser
    {
        ...
        #region Constructors
        public TuringMachineParser(VirtualTuringMachine turingMachine)
        {
            this.turingMachine = turingMachine;
        }

        public TuringMachineParser(VirtualTuringMachine turingMachine, string transitionFormat, string wildCardFormat)
        {
            this.turingMachine = turingMachine;
            ...
            for (int a = 0; a <= lines.Length - 1; a++)
            {
                ...
                if (!(lines[a].StartsWith("//") || lines[a] == ""))
                {
                    ...
                    for (int p = 0; p <= parts.Length - 1; p++)
                    {
                        ...
                        {
                            if (args[4].ToLower() == "l") //Krok vľavo
                            {
                                turingMachine.AddTFunction(new Transition(args[0], args[1], args[2], args[3], Transition.Steps.Left));
                            }
                            else if (args[4].ToLower() == "r") //Krok vpravo
                            {
                                turingMachine.AddTFunction(new Transition(args[0], args[1], args[2], args[3], Transition.Steps.Right));
                            }
                            else if (args[4].ToLower() == "0") //Bez pohybu
                            {
                                turingMachine.AddTFunction(new Transition(args[0], args[1], args[2], args[3], Transition.Steps.NoMove));
                            }
                            else
                            {
                                errors.Add(new TMError("Chybný smer kroku prechodu", "Povolené sú iba hodnoty L,R a 0.", a, p));
                            }
                        }
                    }
                }
