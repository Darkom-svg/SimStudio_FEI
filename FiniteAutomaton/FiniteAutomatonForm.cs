...
namespace FEI.FiniteAutomaton {
	public partial class FiniteAutomatonForm : Form
    {
        ...
        { 
            ...
            int index = Properties.Settings.Default.TransitionFormat;
        ...
        private void bNext_Click(object sender, System.EventArgs e)
        {
            ...
            if (TuringMachine.NoNextStep) stepToolStripButton.Enabled = false;
        ...
        private void RunRealTime()
        {
            lock (TuringMachine)
            {
                ...
                while (!TuringMachine.IsAccepted() && !PrgStop)
                {
        ...
        private void TuringFinish()
        {
            ...
            if (TuringMachine.IsAccepted())
            {
                MessageBox.Show("Konečný automat úspešne dokončil program. (Páska bola prečítaná dokonca a bol dosiahnutý koncový stav.)", "Konečný automat", MessageBoxButtons.OK, MessageBoxIcon.Information);
                tapeAcceptance = AcceptanceStatus.Accept;
            }
            else //Program skončil v inom ako koncovom stave, neexistujú prechodové funkcie pre ďalší chod
            {
                MessageBox.Show("Konečný automat sa nemôže dostať do koncového stav. Neexistujú prechodové funkcie na prechod do ďalšieho stavu.", "Konečný automat", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                tapeAcceptance = AcceptanceStatus.Reject;
            }
            PrgReset = true;
		...
		private void File_Open() {
			if (base.MdiParent == null) {
				...
				this.OpenFileInNewWindow(this.openFileDialog1.FileName);
				return;
			}
