...
namespace FEI.FiniteAutomaton {
	public class FiniteAutomatonParser
    {
        ...
        public FiniteAutomatonParser(VirtualTuringMachine turingMachine)
        {
            this.turingMachine = turingMachine;
        }

        public FiniteAutomatonParser(VirtualTuringMachine turingMachine, string transitionFormat, string wildCardFormat)
        {
            this.turingMachine = turingMachine;
            ...
            for (int a = 0; a <= lines.Length - 1; a++)
            {
                ...
                if (!(lines[a].StartsWith("//") || lines[a] == ""))
                {
                    ...
                    for (int p = 0; p <= parts.Length - 1; p++)
                    {
                        ...
                        {
                            turingMachine.AddTFunction(new Transition(args[0], args[1], args[2], args[1], Transition.Steps.Right));
                        }
                    }
                }
            }            
